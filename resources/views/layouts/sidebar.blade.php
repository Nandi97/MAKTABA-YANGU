<aside
  class="
    md:pt-1
    pt-0
    sticky
    flex flex-col
    md:border-r
    border-base-200
    text-neutral-content
    shadow-lg
    bg-neutral
    md:rounded-box md:h-full
    h-screen
    top-0
    md:p-2
    p-0
    m-0
    z-40
  "
>
  <div>
    <div
      class="
        h-10
        sticky
        navbar
        inset-x-0
        top-0
        z-50
        w-full
        py-1
        transition
        duration-200
        ease-in-out
        md:border-b md:block
        border-base-200
        bg-neutral
        md:rounded-t-lg
      "
    >
      <div class="flex-1 px-4 py-3 mx-2">
        <a href="/">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="inline-block w-5 mr-2 stroke-current"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M12 6.253v13m0-13C10.832
                            5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168
                            18.477 5.754 18 7.5 18s3.332.477 4.5
                            1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0
                            3.332.477 4.5 1.253v13C19.832 18.477 18.247 18
                            16.5 18c-1.746 0-3.332.477-4.5 1.253"
            />
          </svg>
          <B>Maktaba Yangu</B>
        </a>
      </div>
    </div>

    <div>
      <ul class="menu flex flex-col flex-shrink compact md:gap-4 md:pt-3 pt-15">
        <li
          class="
            rounded-full
            md:py-1 md:px-3
            pt-3
            h-15
            w-15
            md:hover:bg-green-500
            outline-green-500
          "
        >
          <a href="/" class="capitalize text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="inline-block md:w-5 md:mr-2 h-6 stroke-current"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1
                            1 0 001
                            1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0
                            001-1v-4a1
                            1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"
              />
            </svg>
            <p>Dashboard</p>
          </a>
        </li>
        <li
          class="
            rounded-full
            md:py-1 md:px-3
            pt-3
            h-15
            w-15
            md:hover:bg-green-500
            outline-green-500
          "
        >
          <a href="/books" class="capitalize text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="inline-block md:w-5 md:mr-2 h-6 stroke-current"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M12 6.253v13m0-13C10.832
                                5.477 9.246
                                5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477
                                5.754 18 7.5
                                18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754
                                5 16.5
                                5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477
                                18.247 18
                                16.5 18c-1.746 0-3.332.477-4.5 1.253"
              />
            </svg>
            <p>Books</p>
          </a>
        </li>
        <li
          class="
            rounded-full
            md:py-1 md:px-3
            pt-3
            h-15
            w-15
            md:hover:bg-green-500
            outline-green-500
          "
        >
          <a href="/users" class="capitalize text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="inline-block md:w-5 md:mr-2 h-6 stroke-current"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M5.121 17.804A13.937
                                    13.937 0
                                    0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3
                                    3 0 11-6
                                    0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118
                                    0z"
              />
            </svg>
            <p>Users</p>
          </a>
        </li>
        <li
          class="
            rounded-full
            md:py-1 md:px-3
            pt-3
            h-15
            w-15
            md:hover:bg-green-500
            outline-green-500
          "
        >
          <a href="/borrowers" class="capitalize text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="inline-block md:w-5 md:mr-2 h-6 stroke-current"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M5.121 17.804A13.937
                                    13.937 0
                                    0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3
                                    3 0 11-6
                                    0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118
                                    0z"
              />
            </svg>
            <p>Borrowers</p>
          </a>
        </li>
        <li
          class="
            rounded-full
            md:py-1 md:px-3
            pt-3
            h-15
            w-15
            md:hover:bg-green-500
            outline-green-500
          "
        >
          <a class="nav-link" href="#" class="pt-5 capitalize text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="inline-block md:w-5 md:mr-2 h-6 stroke-current"
              fill="none"
              viewBox="0 0 24
                                    24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M9 17v-2m3 2v-4m3
                                        4v-6m2
                                        10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1
                                        1 0
                                        01.707.293l5.414 5.414a1 1 0
                                        01.293.707V19a2 2 0
                                        01-2 2z"
              />
            </svg>
            <p>Reports</p>
          </a>
        </li>
      </ul>
    </div>
  </div>
</aside>
